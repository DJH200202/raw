...
def parse_args():
    ...
    parser.add_argument("--layer", type=int, default=14, help="Model layer for attention extraction")
    return parser.parse_args()


def run_2wiki(
    ra_system,
    output_dir="results/2wiki_eval",
    batch_size=8,
    top_k_final=10,
    iter=2,
    prob=0.9,
    ...
    """
    Run RAG system evaluation on 2WikiMultihopQA dataset.
    ...
    # Load 2WikiMultihopQA validation dataset
    with open(os.path.join("data", "hippo", "2wikimultihopqa.json"), "r") as f:
        ds = json.load(f)

    # Load pre-extracted question entities for entity-aware retrieval
    with open(
        os.path.join("results", "question_entities", "2wiki_question_entities.json"),
        "r",
    ) as f:
    ...
    graph_path = "results/saved_graph/2wiki_corpus_entities_graph"
    # Load or create corpus with entity graphs for retrieval
    if not os.path.exists(graph_path):
        os.makedirs(graph_path, exist_ok=True)
        corpus = []
        with open(
            os.path.join("results", "updated_corpus_with_titles.json"),
            "r",
        ) as f:
            corpus = json.load(f)
    ...
    output_file = os.path.join(target_dir, "2wiki_validation_results.jsonl")
...
def main():
    ...
    config = RetrievalConfig(
        ...
        # qa_model=GPTQAModel(model="gpt-4o", layer=args.layer),  # Using GPT-4o for QA
        ...
        layer=args.layer,  # Model layer for attention extraction
    )
    ra_system = RetrievalAugmentation(config=config)

    # Run evaluation on 2WikiMultihopQA dataset
    run_2wiki(
        ra_system,
