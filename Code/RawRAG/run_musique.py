...
def parse_args():
    ...
    parser.add_argument("--attn_topk", type=int, default=25, help="Number of top attention chunks")
    return parser.parse_args()


def run_musique(
    ra_system,
    output_dir="results/musique_eval",
    batch_size=8,
    top_k_final=10,
    iter=2,
    prob=0.9,
    ...
    """
    Run RAG system evaluation on MuSiQue dataset.
    ...
    with open(os.path.join("data", "hippo", "musique.json"), "r") as f:
        ds = json.load(f)

    # Load pre-extracted question entities for entity-aware retrieval
    with open(
        os.path.join("results", "question_entities", "musique_question_entities.json"),
        "r",
    ) as f:
    ...
    graph_path = "results/saved_graph/musique_corpus_entities_graph"
    # Load or create corpus with entity graphs for retrieval
    if not os.path.exists(graph_path):
        os.makedirs(graph_path, exist_ok=True)
        corpus = []
        with open(
            os.path.join("results", "corpus_entities", "musique_corpus_entities.json"),
            "r",
        ) as f:
            corpus = json.load(f)
    ...
    for i in tqdm(range(0, len(ds), batch_size)):
        ...
        # Prepare batch data from MuSiQue paragraph structure
        for j in range(lengths):
            # Extract paragraphs from MuSiQue format
            paragraphs = [para["paragraph_text"] for para in batch[j]["paragraphs"]]
            # Create context by joining paragraphs
            context = "\n\n".join(paragraphs)
            batch_contexts.append(context)
            batch_questions.append(batch[j]["question"])
        ...
        # Store results with answer validation for MuSiQue format
        for j in range(lengths):
            # Check if prediction matches answer or any alias
            if (normalize_answer(predictions[j]) == normalize_answer(batch[j]["answer"])):
                answer = [batch[j]["answer"]]
            elif normalize_answer(predictions[j]) in [normalize_answer(item) for item in batch[j]["answer_aliases"]]:
                answer = [predictions[j]]
            else:
                answer = [batch[j]["answer"]]

            result = {
                "question": batch_questions[j],
                "context": retrieved_contexts[j],
                "answers": answer,
                "pred": predictions[j] ,
            }
    ...
    last_dir = f"iter_{iter}_prob_{prob}_window_top{top_k_final}"
    if filter:
    ...
    output_file = os.path.join(target_dir, "musique_validation_results.jsonl")
